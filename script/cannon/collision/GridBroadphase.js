...
class GridBroadphase extends Broadphase {
  log(string, ...etc) {
    if (!Croquet.Constants.LOGGING["CANNON.GridBroadphase"]) return;

    console.groupCollapsed(`[GridBroadphase-${this.id}] ${string}`, ...etc);
  ...
  init(options = {}) {
    ...
    this.nx = options.nx || 10;
    this.ny = options.ny || 10;
    this.nz = options.nz || 10;
    this.aabbMin = options.aabbMin || new Vec3(100, 100, 100);
    this.aabbMax = options.aabbMax || new Vec3(-100, -100, -100);
  ...
  static get GridBroadphase_collisionPairs_d() {
    const name = "CANNON.GridBroadphase.GridBroadphase_collisionPairs_d";
    ...
    for (let i = 0; i !== N; i++) {
      ...
      switch (si.type) {
        ...
        case PLANE: {
          ...
          const d = GridBroadphase.GridBroadphase_collisionPairs_d;
  ...
  destroy() {
    ...
    this.aabbMin.destroy();
    this.aabbMax.destroy();
  }
}
GridBroadphase.register("CANNON.GridBroadphase");
